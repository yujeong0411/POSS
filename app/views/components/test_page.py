...
class TestPage(QWidget):
    ...
    def __init__(self, main_window):
        ...
        # ===== 3. UI 초기화 (시그널 연결은 하지 않음) =====
        self.init_ui()

        # ===== 4. 초기 시그널 연결은 여기서 하지 않음 =====
        # connect_signals()는 컨트롤러 설정 후 또는 레거시 모드에서 명시적으로 호출
        print("TestPage 초기화 완료 - 시그널 연결은 보류")
    ...
    def init_ui(self):
        ...
        # 타이틀 레이블
        title_label = QLabel("Test Result")  # 제목 변경으로 구분
        ...
        kpi_title.setStyleSheet(
            f"color: #333; border: none; font-family: {bold_font}; font-size: {f(14)}px; font-weight: bold; margin-left: {w(10)}px")
        ...
        # 직접 픽셀 크기 지정 (1:1 비율)
        right_top_horizontal_splitter.setSizes([500, 500])
        ...
        button_group_layout.setAlignment(Qt.AlignCenter)

        # 5) 탭 버튼/페이지 생성
        self.tab_manager.create_tab_buttons(button_group_layout)
        self.viz_buttons = self.tab_manager.buttons
        ...
        # 스플리터를 메인 레이아웃에 추가
        result_layout.addWidget(main_horizontal_splitter, 1)

    """
    컨트롤러 설정
    """
    def set_controller(self, controller, defer_signal=False):
        """컨트롤러 설정 및 MVC 모드 활성화"""
        self.controller = controller
        self._mvc_mode = True
        print("TestPage: MVC 모드로 전환됨")
    ...
    def _setup_widget_references(self):
        ...
        if shipment_tab:
            ...
            # *** 중요: 시그널 연결은 _signals_connected 상태를 확인한 후에만 ***
            if (hasattr(self.shipment_widget, 'shipment_status_updated') and
                    self._signals_connected and
                    not hasattr(self, '_shipment_signal_connected')):
                self.shipment_widget.shipment_status_updated.connect(self.on_shipment_status_updated)
                self._shipment_signal_connected = True
                print("Shipment 위젯 시그널 연결됨")
        ...
        if material_tab:
            ...
            # *** 중요: 시그널 연결은 _signals_connected 상태를 확인한 후에만 ***
            if (hasattr(self.material_widget, 'material_shortage_updated') and
                    self._signals_connected and
                    not hasattr(self, '_material_signal_connected')):
    ...
    def connect_signals(self):
        ...
        if self._signals_connected:
            print("ResultPage: 시그널이 이미 연결되어 있음 - 중복 연결 방지")
            return

        if self._mvc_mode and hasattr(self, 'controller') and self.controller:
            print("ResultPage: MVC 모드 시그널 연결")
            # MVC 모드: 컨트롤러를 통한 처리만
            ...
            if hasattr(self, 'left_section') and hasattr(self.left_section, 'itemModified'):
                ...
                print("left_section itemModified 시그널 연결")

        self._signals_connected = True
        print("ResultPage: 시그널 연결 완료")

    def disconnect_signals(self):
        """시그널 연결 해제"""
        if not self._signals_connected:
            print("TestPage: 시그널이 연결되어 있지 않음")
            return
        ...
        try:
            if self._mvc_mode and hasattr(self, 'controller') and self.controller:
                # MVC 모드 시그널 해제
                if hasattr(self.controller.model, 'modelDataChanged'):
                    self.controller.model.modelDataChanged.disconnect(self.update_ui_from_model)
                    print("MVC 모드 시그널 연결 해제")
            else:
                # 레거시 모드 시그널 해제
                if hasattr(self, 'left_section'):
                    if hasattr(self.left_section, 'viewDataChanged'):
                        self.left_section.viewDataChanged.disconnect(self.on_data_changed)
                    if hasattr(self.left_section, 'itemModified'):
                        self.left_section.itemModified.disconnect(self.on_item_data_changed_legacy)
                    print("레거시 모드 시그널 연결 해제")

            self._signals_connected = False
            print("TestPage: 시그널 연결 해제 완료")
    ...
    def on_data_changed(self, data):
        """데이터 변경 처리 - 모든 모드에서 사용"""
        print("on_data_changed 호출됨")
        self.result_data = data

        # 위젯 참조가 없으면 다시 설정
        if self.plan_maintenance_widget is None:
            self._setup_widget_references()
        """레거시 모드에서만 사용되는 데이터 변경 처리"""
        if self._mvc_mode:
            print("WARNING: MVC 모드에서 on_data_changed가 호출됨 - 이는 비정상적입니다")
            return

        print("on_data_changed 호출됨 (레거시 모드)")
        self.result_data = data
    ...
    def on_item_data_changed_legacy(self, item, new_data):
        """레거시 모드에서만 사용되는 아이템 데이터 변경 처리"""
        if self._mvc_mode:
            print("WARNING: MVC 모드에서 on_item_data_changed_legacy가 호출됨")
            return

        print("on_item_data_changed_legacy 호출됨 (레거시 모드)")
        ...
        if 'Qty' in new_data and pd.notna(new_data['Qty']):
            ...
            if all([line, time is not None, item_code, new_qty is not None]):
                # 계획 유지율 위젯 업데이트
                if hasattr(self, 'plan_maintenance_widget') and self.plan_maintenance_widget:
    ...
    def update_ui_from_model(self, model_df=None):
        ...
        # MVC 모드가 아니면 경고만 출력하고 계속 진행
        if not self._mvc_mode:
            print("WARNING: 레거시 모드에서 update_ui_from_model이 호출됨 - 하지만 처리 계속")
    ...
    def _update_ui_components(self, data):
        ...
        if self.material_analyzer and hasattr(self.material_analyzer, 'shortage_results'):
            self.update_left_widget_shortage_status(self.material_analyzer.shortage_results)
        if hasattr(self, 'left_section') and hasattr(self.left_section,
                                                     'shipment_failure_items') and self.left_section.shipment_failure_items:
        ...
        try:
            if data is not None and not data.empty:
                ...
                # 시각화 데이터 업데이트
                self._update_visualization_data(data)
    ...
    def _update_visualization_data(self, data):
        """시각화 데이터 업데이트 로직"""
        # === 핵심: 비교차트 조건 분기 ===
        has_user_adjustments = False
        if self.controller and hasattr(self.controller, 'model'):
    ...
    def test_mvc_mode(self, controller):
        """MVC 모드 테스트"""
        print("=== MVC 모드 테스트 시작 ===")
        self.set_controller(controller)
        print("=== MVC 모드 테스트 완료 ===")
    ...
    def test_signal_duplicate(self):
        """시그널 중복 연결 테스트"""
        print("=== 시그널 중복 연결 테스트 시작 ===")
        print(f"첫 번째 연결 전 상태: _signals_connected = {self._signals_connected}")

        # 첫 번째 연결 시도
        self.connect_signals()
        print(f"첫 번째 연결 후 상태: _signals_connected = {self._signals_connected}")

        # 두 번째 연결 시도 (중복 방지되어야 함)
        self.connect_signals()
        print(f"두 번째 연결 후 상태: _signals_connected = {self._signals_connected}")

        print("=== 시그널 중복 연결 테스트 완료 ===")
        return self

    def get_current_mode(self):
        """현재 모드 확인"""
        return "MVC" if self._mvc_mode else "Legacy"

    def get_signal_status(self):
        """시그널 연결 상태 확인"""
        return "Connected" if self._signals_connected else "Disconnected"

    def print_status(self):
        """현재 상태 출력"""
        print(f"TestPage 상태:")
        print(f"  - 모드: {self.get_current_mode()}")
        print(f"  - 시그널: {self.get_signal_status()}")
        print(f"  - 컨트롤러: {'있음' if self.controller else '없음'}")
        return self